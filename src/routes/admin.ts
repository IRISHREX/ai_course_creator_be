...
const backupTables = [
  ...
  { name: "topics", columns: ["id", "course_id", "slug", "unit", "order_index", "title", "summary", "content", "quiz", "mindmap", "visualization", "difficulty_level", "generation_status", "created_at", "updated_at"] },
...
async function getBackupTables(): Promise<BackupTable[]> {
  ...
  return [
    ...
    { name: "topics", rows: topics.map((item) => row({ id: item.id, course_id: item.courseId, slug: item.slug, unit: item.unit, order_index: item.orderIndex, title: item.title, summary: item.summary, content: item.content, quiz: item.quiz, mindmap: item.mindmap, visualization: item.visualization, difficulty_level: item.difficultyLevel, generation_status: item.generationStatus, created_at: item.createdAt, updated_at: item.updatedAt })) },
