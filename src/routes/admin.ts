...
const backupTables = [
  ...
  { name: "topics", columns: ["id", "course_id", "slug", "unit", "order_index", "title", "summary", "content", "translations", "quiz", "mindmap", "presentation", "visualization", "difficulty_level", "generation_status", "created_at", "updated_at"] },
  { name: "topic_versions", columns: ["id", "topic_id", "title", "summary", "content", "quiz", "mindmap", "presentation", "visualization", "note", "created_by", "created_at"] },
...
async function getBackupTables(): Promise<BackupTable[]> {
  ...
  return [
    { name: "users", rows: users.map((item: any) => row({ id: item.id, email: item.email, display_name: item.displayName, created_at: item.createdAt })) },
    { name: "user_roles", rows: roles.map((item: any) => row({ id: item.id, user_id: item.userId, role: item.role, created_at: item.createdAt })) },
    { name: "courses", rows: courses.map((item: any) => row({ id: item.id, slug: item.slug, title: item.title, description: item.description, cover_emoji: item.coverEmoji, order_index: item.orderIndex, source_text: item.sourceText, generation_status: item.generationStatus, tags: item.tags, mindmap: item.mindmap, toc: item.toc, created_at: item.createdAt, updated_at: item.updatedAt })) },
    { name: "topics", rows: topics.map((item: any) => row({ id: item.id, course_id: item.courseId, slug: item.slug, unit: item.unit, order_index: item.orderIndex, title: item.title, summary: item.summary, content: item.content, translations: item.translations || [], quiz: item.quiz, mindmap: item.mindmap, presentation: item.presentation, visualization: item.visualization, difficulty_level: item.difficultyLevel, generation_status: item.generationStatus, created_at: item.createdAt, updated_at: item.updatedAt })) },
    { name: "topic_versions", rows: versions.map((item: any) => row({ id: item.id, topic_id: item.topicId, title: item.title, summary: item.summary, content: item.content, quiz: item.quiz, mindmap: item.mindmap, presentation: item.presentation, visualization: item.visualization, note: item.note, created_by: item.createdBy, created_at: item.createdAt })) },
    { name: "bookmarks", rows: bookmarks.map((item: any) => row({ id: item.id, user_id: item.userId, topic_id: item.topicId, course_id: item.courseId, page_index: item.pageIndex, word_index: item.wordIndex, label: item.label, created_at: item.createdAt })) },
    { name: "topic_progress", rows: progress.map((item: any) => row({ id: item.id, user_id: item.userId, topic_id: item.topicId, viewed: item.viewed, passed: item.passed, attempts: item.attempts, best_quiz_score: item.bestQuizScore, updated_at: item.updatedAt })) },
    { name: "course_pyq", rows: pyqs.map((item: any) => row({ id: item.id, course_id: item.courseId, topic_id: item.topicId, question: item.question, answer: item.answer, marks: item.marks, year: item.year, source: item.source, ingestion_source: item.ingestionSource, order_index: item.orderIndex, created_at: item.createdAt, updated_at: item.updatedAt })) },
    { name: "pyq_topics", rows: pyqTopics.map((item: any) => row({ id: item.id, pyq_id: item.pyqId, topic_id: item.topicId, created_at: item.createdAt })) },
  ];
}
