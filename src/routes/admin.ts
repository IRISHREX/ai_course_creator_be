...
async function getBackupTables(): Promise<BackupTable[]> {
  ...
  return [
    ...
    { name: "topics", rows: topics.map((item: any) => row({ id: item.id, course_id: item.courseId, slug: item.slug, unit: item.unit, order_index: item.orderIndex, title: item.title, summary: item.summary, content: item.content, translations: item.translations || [], quiz: item.quiz, mindmap: item.mindmap, visualization: item.visualization, difficulty_level: item.difficultyLevel, generation_status: item.generationStatus, created_at: item.createdAt, updated_at: item.updatedAt })) },
    { name: "topic_versions", rows: versions.map((item: any) => row({ id: item.id, topic_id: item.topicId, title: item.title, summary: item.summary, content: item.content, quiz: item.quiz, mindmap: item.mindmap, visualization: item.visualization, note: item.note, created_by: item.createdBy, created_at: item.createdAt })) },
